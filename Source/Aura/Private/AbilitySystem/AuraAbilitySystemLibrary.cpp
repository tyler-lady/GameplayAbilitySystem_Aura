	...
	const FVector& SphereOrigin)
{
	...
	if (const UWorld* World = GEngine->GetWorldFromContextObject(WorldContextObject, EGetWorldErrorMode::LogAndReturnNull))
	{
		...
		{
			if ( Overlap.GetActor()->Implements<UCombatInterface>() && !ICombatInterface::Execute_IsDead(Overlap.GetActor()) ) // The second(2nd) condition isn't checked if the first(1st) isn't passed
			{
