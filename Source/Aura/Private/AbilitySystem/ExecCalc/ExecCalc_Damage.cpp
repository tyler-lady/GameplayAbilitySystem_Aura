...
struct AuraDamageStatics // Raw internal struct to be used in THIS c++ file
{
	DECLARE_ATTRIBUTE_CAPTUREDEF(Armor); // Attribute Capture declaration
	...
	AuraDamageStatics()
	{
		DEFINE_ATTRIBUTE_CAPTUREDEF(UAuraAttributeSet, Armor, Target, false); // Attribute Capture definition
	...
	FGameplayEffectCustomExecutionOutput& OutExecutionOutput) const
{
	 ...
	 * Also responsible for considering DamageType Resistances
	 * If we add a Set By Caller Magnitude where the key is a dmg type
	 *	the dmg and resistance will be considered
	 */
	float Damage = 0.f;
	for(const TTuple<FGameplayTag, FGameplayTag>& Pair : FAuraGameplayTags::Get().DamageTypesToResistances)
	{
		...
		checkf(AuraDamageStatics().TagsToCaptureDefs.Contains(ResistanceTag), TEXT("TagsToCaptureDefs doesn't contain Tag: [%s] in ExexCalc_Damage"), *ResistanceTag.ToString());
		...
		DamageTypeValue*= (100.f - Resistance) / 100.f; // If Resistance = 10; 100-10=90/100=.9 - Essentially reducing DmgTypVal by 10%
	 ...
	 * with the math setup like this... 100 becomes MAX Armor Pen
	 * Can scale SourceArmorPen by multiplying it by a value
	 * Essentially calculating Armor Vs. Armor Pen for true Dmg value
	 */
